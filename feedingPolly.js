...
const displayQuestions = () => {
    var loc=require('./libs/getNotes');
    loc.getNotes('./demo/ppt/notesSlides/').then(data=>{
        //console.log("hello");
        var text_dic=data;
        var texts=[];
    
        for(i in text_dic)
        {   var musicfile= 'music'+text_dic[i][0];
            console.log(text_dic[i][1]);
            setupPolly(text_dic[i][1]);

            generateAudio(pollyParams,musicfile, config.audioPath).then(() => {
                console.log(success('\n Audio Generated! \n'));
